...
% 			invoke(obj.obj_control_design,'SetVariable',piTHZ,Theta_freewheel); 
		...
		function [Efficiency,Average_torque,Current_density_RMS] = Simulate(obj)
			...
			%% Get parameters from PC-SRD
 			Average_torque = invoke(obj.obj_control_design,'GetVariable',piTORQSH);            % Average machine torque              % RMS (root mean square) phase current
...
% 		function SimulateFrequentlyPoint(obj,speed_of_machine)
% % 			if speed_of_machine == 1
% % 				[Th0,ThC,iHi,HBAmps2] = obj.ConvertParameter(test_Th0,test_ThC,test_iHi,20.6,6825);
